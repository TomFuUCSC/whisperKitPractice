...
struct ContentView: View {
    ...
    var lastChatCount: Int = 0
    @State var role = "Your role is to be an iOS developer in an interview with me. You need to answer my technical questions. And every answers no longer than 5 sentences. make  keyword different color for easy to read."
    ...
    let openAI = OpenAI(apiToken: "XXXXToken")
    ...
    var body: some View {
        NavigationSplitView(columnVisibility: Binding.constant(.all)) {
            ...
            let markDown = gptResult.joined()
            ScrollView(.vertical) {
                Markdown(markDown)
                    .markdownTheme(.gitHub)
            }

                // let _ = print("🙂", gptResult.last?.text)
            }
        } detail: {
            VStack {
                #if os(iOS)
...
//                }
        ...
        if let query = unconfirmedSegments.last?.text, !query.isEmpty {
            let message = query + "and Make your answer concise, should not longer 5 sentences.mark keyword different color for easy to read."
             try await ask(str: message)
        }
    }
    ...
    private func ask(str: String) async throws {
        print("🙂", str)
        let query = ChatQuery(messages: [.init(role: .user, content: str)!], model: .gpt4_o)
        let result = try await openAI.chats(query: query)
        let reply = result.choices.reduce("") { $0 + ($1.message.content?.string ?? "") }
        if gptResult.count >= 2 {
            gptResult = [reply]
        } else {
            gptResult.append(reply)
        }
    }
}
